...
class RestritoController extends Zend_Controller_Action{
	...
	function pendenciatccAction(){
		$sql = "SELECT id FROM pendenciaTCC WHERE idCadastro={$this->view->user->idCadastro}"; //echo $sql;
		$this->view->tcc = $this->fc->dbReader($this->db, "SELECT id FROM pendenciaTCC_2014 WHERE idCadastro={$this->view->user->idCadastro}");
		//$this->view->id = (int)$this->view->tcc->id;
		if((int)$this->view->tcc->id == 0)
			$this->view->message = "Desculpe mas seu cadastro não está relacionado na lista para rematrícula.\n\nEntre em contato com mariadasdores@amfar.com.br\npara verificação.\n\n";
		
		if ($this->_request->isPost()){
			try{
				
				$curso = $this->fc->dbReader($this->db, "SELECT * FROM pendenciaTCC_2014 WHERE id={$_POST['id']}");
				//$this->fc->debug($curso);
				//die();
				$sql = "SELECT idAlunoCursos FROM alunosCursos WHERE (idAluno={$this->view->user->idCadastro}) AND (idCurso=150)";
				$found = (int)$this->fc->dbReader($this->db, $sql)->idAlunoCursos;
				//$this->fc->debug($_POST); die($sql);
				if($found > 0){
				...
				$_boleto = array(
					...
					'status' => 2,
					'idCentroCusto' => 1,
					'numDoc' => sprintf('%04d15001', $this->view->user->idCadastro, $this->view->id),
					'vcto' => date("m.d.Y", time() + (3 * 86400)),
					'parcela' => 1,				
					);
	...
	function monitorAction(){
		try{
			$this->view->where = "RESTRITO/SELEÇÃO DE MONITORES";
			$this->view->liberado = true;
			$sql = sprintf("SELECT * FROM alunosCursos WHERE (idAluno=%d) AND (idCurso=158)", $this->view->user->idCadastro); //echo $sql;
			$this->view->curso = $this->fc->dbReader($this->db, $sql);
			//$this->fc->debug($this->view->curso);
	
			$this->view->matricula = $this->fc->dbReader($this->db, sprintf("SELECT TOP (1) valor, pgto FROM financeiro WHERE (idCadastro=%d) AND (idCurso=158) AND (numDoc='%04d15800')",
				$this->view->user->idCadastro, $this->view->user->idCadastro));

			if ($this->_request->isPost()){
				//$this->fc->debug($_POST);
				//$this->fc->debug($_FILES);
				if(empty($_FILES['userFile']['name'])){
					throw new Exception("É obrigatório o envio do currículo em WORD!\n\n");
					}
				$filtro = "doc|docx";
				$baseDir = sprintf("..%s/public/curriculos", $this->view->baseUrl);
				$_upfile = $_FILES['userFile'];
				$filename = basename($_upfile['name']);
				$t = explode('.', strtolower($filename));							
				$target = sprintf('%s/%s.%s', $baseDir, date('YmdHis'), basename($_upfile['name']));						

				if($_upfile['size'] > 8192000)
					throw new Exception("O tamanho do arquivo não pode ser\nsuperior a 8192000kb ou seja 8MB.\n\n");
				elseif(!stristr($filtro, $t[count($t)-1]))
					throw new Exception("O arquivo enviado $filename é inválido.\n\nEnvie somente arquivos no formato $filtro");
								
				if(!move_uploaded_file($_upfile['tmp_name'], $target))
					throw new Exception("Erro transferindo arquivo $filename para o servidor !\n\n");
					
				$idCadastro = (int)$_POST['idCadastro'];
				$cadastro = $this->fc->dbReader($this->db, "SELECT nome, email, fone, fone2 FROM cadastro WHERE (idCadastro=$idCadastro)");
				$id = (int)$_POST['id'];
				$this->db->query("UPDATE alunosCursos SET monitor=1 WHERE idAlunoCursos=$id");
				$this->view->curso->monitor = 1;
				$this->view->liberado = false;
				$ebody="
				<html>
					<header>
						<style>
					...
					<body>
						<a href='http://www.amfar.com.br'><img src='topoEmail.jpg' border='0'/>	</a>
						<table class='econtato' cellpadding='1' cellspacing='1' border='0'>
							<tr>
							...
							<tr>
								...
								<td><b>$cadastro->email</b></td>
							</tr>
							<tr>
								<td class='label'>Telefone:</td>
								<td><b>$cadastro->fone</b></td>
							</tr>
							<tr>
								<td class='label'>Celular:</td>
								<td><b>$cadastro->fone2</b></td>
							</tr>
						</table>
					</body>
				</html>";
				$mail = new htmlMimeMail();
				$mail->setReturnPath('webmaster@amfar.com.br');
				$mail->setFrom("Associação Mineira de Farmacêuticos <webmaster@amfar.com.br>");
				$mail->setReplayTo($cadastro->email);
				//$mail->setCc("Webmaster <webmaster@sbrafh.org.br>");
				//$mail->setBcc('Webmaster <webmaster@sobrafo.org.br>');
				$mail->setSubject("Candidato(a) a Monitor");
				// para o usuario envia tabela de distribuidores. esta mesma copia enviar para ridgid e para os supervisores
				$text = strip_tags($ebody);
				$mail->setHtml($ebody, $text, "./public/images/"); // envia html completo com todos os distribuidores			

				$attachment = $mail->getFile($target);
				$type = $this->mime_content_type($target);
				$mail->addAttachment($attachment, basename($target), $type);

				$result = $mail->send(array('mariadasdores@amfar.com.br'));  // envia copia para o usuario
			   $this->view->message = "Email enviado com sucesso.\n\nResponderemos em breve.\n\nObrigado.\n\n";
			   $_POST = null;								
				//application/msword
				}
			elseif($this->view->curso->monitor == 1){
				$this->view->liberado = false;
				throw new Exception("Você já enviu seu currículo.\n\nPor favor aguarde uma resposta!\n\n");
				}
			

			if(empty($this->view->curso)){
				$this->view->liberado = false;
				throw new Exception("Você não está matriculado(a) neste curso!\n\n");
				}
			

			if($this->view->matricula->pgto == ''){
				$this->view->liberado = false;
				throw new Exception("A sua matrícula está pendente de pagamento!\n\n");
				}			

			if(date('Ymd') < '20150310'){
				$this->view->liberado = false;
				throw new Exception("O prazo para inscrição inicia em 10/03/2015!\n\n");
				}

			if(date('Ymd') > '20150325'){
				$this->view->liberado = false;
				throw new Exception("O prazo para inscrição encerrou em 25/03/2015!\n\n");
				}
			}
		catch(Exception $ex){			
			$this->view->message = $ex->getMessage();
