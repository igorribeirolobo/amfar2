...
class CadastroController extends Zend_Controller_Action{
	...
	function indexAction(){	
		$this->view->where = "Ficha de Cadastro";
		...
		$this->view->estados = $this->fc->dbReader($this->db, "SELECT uf, upper(estado) AS estado FROM estados ORDER BY uf", true);
		}
	...
	function insertmailingAction(){
		$table = "mailing_ismp";
		$_target = sprintf("..%s/public/$table.txt", $this->view->baseUrl);
		$fd = fopen ($_target, "r");
		while (!feof ($fd)) {
			$buffer[] = fgets($fd, 4096);	
			}
		fclose ($fd);
		define('logfile', str_replace('.txt', sprintf('_%s.log', date('Ymd')), $_target));		
		$fpLog = fopen(logfile, "w");
		fwrite($fpLog, date('d.m.Y H:i:s')." - processando arquivo $_target\r\n\r\n");
		foreach ($buffer as $_row => $str):
			$temp = explode(';', $str);
			foreach ($temp as $email):
				$email = trim($email);
				if(!empty($email)):
					$sql = "INSERT INTO $table SET email=trim('$email')";				
					fwrite($fpLog, sprintf("%s - Inserindo: %s\r\n", date('d.m.Y H:i:s'), $sql));
					try{
						echo "$sql<br />";
						$this->db2->query($sql);						
						}
					catch(Exception $ex){
						fwrite($fpLog, sprintf("%s - %s\r\n", date('d.m.Y H:i:s'), $ex->getMessage()));
						}
				endif;
			endforeach;	
		endforeach;
		fwrite($fpLog, date('d.m.Y H:i:s')." - Processo encerrado ");
		fclose($fpLog);
		die("encerrado");	
		}


	function checkmailingAction(){
		$table = "mailing_newton";
		$query = $this->fc->dbReader($this->db2, "SELECT * FROM $table", true);
		foreach ($query as $_row => $email):
			$found = (int)$this->fc->dbReader($this->db2, "SELECT id FROM mailing_ismp WHERE email='$email->email'")->id;
			if($found):
				$sql = "UPDATE mailing_ismp SET status=-2 WHERE id='$email->id'";				
				try{
					echo "$sql<br />";
					$this->db2->query($sql);						
					}
				catch(Exception $ex){
					echo sprintf("%s - %s<br />", date('d.m.Y H:i:s'), $ex->getMessage());
					}
			endif;
		endforeach;
		die("encerrado");	
