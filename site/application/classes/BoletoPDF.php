...
class BoletoPDF extends FPDF {
	...
	function BodyTable() {		
		...
		$this->SetLineWidth(.3);		
		$this->SetFont('','B', 9);
		$this->Cell(0, $height, 'Instruções para Impressão', $border, 0, 'C');
		$this->Ln();
		$this->Ln();
		...
		$this->SetFont('','', 8);
		$this->Cell(88);
		$this->Cell(0, 3.8, 'Sociedade Brasileira de Farmácia Hospitalar (SBRAFH)');
		$this->Ln();
		$this->Cell(88);		
		$this->Cell(0, 3.8, '01.456.716/0001-60 - Telefone: (11) 5083-4297');
		$this->Ln();
		$this->Cell(88);
		$this->Cell(0, 3.8, 'Rua Vergueiro, 1855 - 12º andar - Vila Mariana');
		$this->Ln();
		$this->Cell(88);
		$this->Cell(0, 3.8, 'CEP 04101-000 - São Paulo - SP');
		$this->Image('..'.$this->_boleto->logoEmpresa, 10, 53, 86);
		...
		$this->Ln();
		$this->Image('..'.$this->_images.'/boleto/logobradesco.jpg', 10, 78, 42);
		...
		$this->SetFont('','B', 8);
		$this->Cell( 2, 3.9);
		$this->Cell(81, 3.9, 'Sociedade Brasileira de Farmácia Hospitalar (SBRAFH)',0,0,'L');
		$this->Cell(40, 3.9, $this->_boleto->agenciaConta,0,0,'L');
		$this->Cell(12, 3.9);
		$this->Cell(17, 3.9, '001');
		$this->Cell( 0, 3.9, $this->_boleto->nossoNumero, 0, 0, 'R');
		$this->Ln();
		...
		$this->Cell(0, $height, $this->_boleto->linhaDigitavel, 0, 0, 'R');
		$this->Ln();
		$this->Image('..'.$this->_images.'/boleto/logobradesco.jpg', 10, 154, 42);
		...
		$this->SetFont('','B', 8);
		$this->Cell(  2, $h2, '', 0, 0, 'L');
		$this->Cell(134, $h2, 'Sociedade Brasileira de Farmácia Hospitalar (SBRAFH)', 0, 0, 'L');
		...
		$this->Cell( 0, $h2, $this->_boleto->nossoNumero, 0, 0, 'R');
		$this->Ln();
		...
		$this->SetFont('','B', 8);
		$this->Cell( 2, $h2, '', 0, 0, 'L');
		$this->Cell(134, $h2, '- SR. Caixa, não receber após o vencimento', 0, 0, 'L');
		$this->Ln();
		
		$this->SetFont('','B', 8);
		$this->Cell( 2, $h2, '', 0, 0, 'L');
		$this->Cell(134, $h3, '- Utilizar opção Título/Boleto para pagto. via internet ou caixa automático.', 0, 0, 'L');
		$this->SetFont('','', 6);
		$this->Cell(0, $h3, '(-) Outras Deduções', 0, 0, 'L');
		$this->Ln();
		$this->SetFont('','B', 8);
		$this->Cell( 2, $h2, '', 0, 0, 'L');
		$this->Cell(134, $h2, '- Em caso de dúvidas entre em contato com atendimento@sbrafh.org.br', 0, 0, 'L');
		$this->Ln();
		
		$this->Cell( 2, $h2, '', 0, 0, 'L');
		$this->Cell(134, $h3, '  ou pelo telefone: 11 5083-4297', 0, 0, 'L');
		$this->SetFont('','', 6);
	...
	public function getSetting(){
		...
		$this->_boleto->logoEmpresa= "$this->_images/boleto/logoEmpresa.jpg"; // logo que vai no topo do boleto
		$this->_boleto->codBanco = '237'; // banco
		...
		$this->_boleto->agencia = '3168'; // agencia
		$this->_boleto->agenciaDv = '3168-2';	// dv da agencia
		$this->_boleto->conta = "0006550"; 	// Num da conta, 7 digitos sem dv
		$this->_boleto->contaDv = "0006550-1"; 	// dv da conta	
		$this->_boleto->carteira = "09";  // Código da Carteira
		$this->_boleto->quantidade = "001";
		$this->_boleto->especieDoc = "DS";
		$this->_boleto->aceite = "";		
		$this->_boleto->nossoNumero = sprintf('%02d%011d', $this->_boleto->carteira, $this->_boleto->numDoc);
		$this->_boleto->nossoNumeroDv = $this->digitoVerificador_nossonumero($this->_boleto->nossoNumero);
		
		$this->_boleto->codBarDv = $this->digitoVerificador_barra($this->_boleto->codBanco . $this->_boleto->moeda . $this->_boleto->fatorVcto . 
			sprintf('%010d', str_replace('.','',str_replace(',','', $this->_boleto->valor))) . $this->_boleto->agencia . $this->_boleto->nossoNumero . $this->_boleto->conta . '0', 9, 0);

		$this->_boleto->codBar = $this->_boleto->codBanco . $this->_boleto->moeda . $this->_boleto->codBarDv . $this->_boleto->fatorVcto . 
			sprintf('%010d', str_replace('.','',str_replace(',','', $this->_boleto->valor))) . $this->_boleto->agencia . $this->_boleto->nossoNumero . $this->_boleto->conta . '0';

		die("this->_boleto->codBar = $this->_boleto->codBar");
		$this->_boleto->nossoNumero = substr($this->_boleto->nossoNumero,0,2) . '/' . substr($this->_boleto->nossoNumero,2) . '-' . $this->_boleto->nossoNumeroDv;
		$this->_boleto->agenciaConta = $this->_boleto->agenciaDv . ' / ' . $this->_boleto->contaDv;
	...
	function setbar($img, $narrow=true){
		...
		$this->Image("..$this->_images/boleto/$img.png", $this->_cur_x, $cur_y, $width, $height);
		$this->_cur_x += $width;	
		}



	private function fbarcode(){
		$valor = $this->_boleto->codBar;
		$fino = 1;
		$largo = 3;
		$altura = 50;
		$cur_x = 12;
		...
		while(strlen($texto) > 0):
			...
			for($i=1;$i<11;$i+=2):
				if (substr($f,($i-1),1) == "0"){
					...
					$this->setbar('p1', false);
					}
				if (substr($f,$i,1) == "0") {
					$this->setbar('b1', true);
					}
				else{
					$this->setbar('b1', false);
					}
			endfor;
		endwhile;
