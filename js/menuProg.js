...
if(typeof(st_js)=="undefined") {
	...
	st_gc=st_rl=st_cl=st_ct=st_cw=st_ch=st_cm=st_cp=st_ci=st_load=st_scr=0;st_ht="";
	st_ims=[];st_ms=[];stusrld=[];
...
function stm_bp(l,a) {
	var m=st_ms[st_cm],p,i=m.ps.length?m.ps[st_cp].is[st_ci]:0;st_cp=m.ps.length;st_ci=0;m.ps[st_cp]=p={is:[],mei:st_cm,ppi:st_cp,ids:"Stm"+st_cm+"p"+st_cp+"i",par:i,tmid:0,cuit:0,issh:0,isst:!st_cp&&!m.mtyp,isck:!st_cp&&(m.mcks&1),exed:0,pver:a[0],pdir:a[1],poffx:a[2],poffy:a[3],pspc:a[4],ppad:a[5],plmw:a[6],prmw:a[7],popc:a[8],pstp:a[14],psds:nIEW?a[15]:0,pscl:a[16],pbgc:a[17],pbgi:stbuf(stgsrc(a[18],0)),pbgr:stREP[a[19]],pbds:stBDS[a[20]],ipbw:a[21],pbdc:(!nDM||nNN4)?a[22].split(/\s/gi)[0]:a[22],args:a.slice(0)};p.peff=[stgeff(a[9],a[10],a[13],p),stgeff(a[11],a[12],a[13],p)];if(st_cp)	p.par.sub=p;p.zind=!st_cp?1000:stgpar(p.par).zind+1;p.pbgd=stgbg(p.pbgc,p.pbgi,p.pbgr);if(nIEW)p.effn=[p.peff[0]?p.peff[0].split(" ").length:0,p.peff[1]?p.peff[1].split(" ").length:0];
	eval(l+"=p");
	}
...
function stm_ai(l,a) {
	...
	p.is[st_ci]=i;
	i.ityp=a[0];
	i.mei=st_cm;i.ppi=st_cp;
	i.iti=st_ci;i.ids=p.ids + st_ci+"e";
	i.sub=0;
	i.tmid=0;i.iwd=0;i.iht=0;i.cstt=0;i.lock=0;
	if(m.midn&1)
		sthili(m,i);
	eval(l+"=i");
	}
	...
	if(nNN4)
		...
		while(ly) {
			...
			if(nIEW&&stisit(ip) && ly.id == stgpar(ip).ids+"SC") {
				l -= ly.scrollLeft;
				t -= ly.scrollTop;}
			ly=(nKQ2 || navigator.appVersion.indexOf("Safari") >= 0) && ly.style && ly.style.position && ly.style.position.toLowerCase() == "absolute" ? 0 : ly.offsetParent;}
			if(nIEM)
...
function stbuf(s) {
	if(s&&stBIMG) {
		var i=new Image();
		st_ims[st_ims.length]=i;i.src=s; }
	return s; }
...
function stgfrm(m) {
	...
	for(var j = 0; j < a.length; ++j) {
		w+="."+a[j];if(typeof(eval(w))=="undefined")
		return 0;}
...
function stgspc(p,d) {
	var m=stgme(p),w=m.sfrm;return p.par&&!p.par.ppi&&w!=window&&stdmld(w)&&w.stmenu(m.mnam).mcfd==d?stCFSP:stSCSP;}
...
function stscit(p) {
	var d=p.scpx;p.scid=0;with(stgme(p).mscm[p.pver])
	if(d) {
...
function sthili(m,i) {
	with(m)
		if(i.ityp!=6&&stcurl(i,midn&0x02000000)) {
			if(midn&0x01000000)
			i.iurl="";
			do {
				...
				if(midn&4) {
					i.itxf[0]=i.itxf[1];i.itxd[0]=i.itxd[1];
					}
...
function stcurl(i,c) {
	...
	var t=stgtgt(i);var u=t?t.location.href:"",h=i.iurl;
	if(!c) {
		u=u.toLowerCase();h=h.toLowerCase();
		}
	...
	if(u&&h) {
		if(!h.indexOf("file:////"))
			...
			if(h.charAt(0)=="/") {
				...
				re.exec(u);var s=RegExp.$1,n,r;r=s&&!u.indexOf(s)?s.substr(0,s.length-1):"";
				h=r?r+h:"";
				}
