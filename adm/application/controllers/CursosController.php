...
class cursosController extends Zend_Controller_Action{
	function init(){
		...
		$this->id = $this->view->id = (int)$this->_request->getParam('id',0);
	...
	function contratoAction(){
		...
		//$sql = "SELECT titulo, inicio, final, valor, parcelas, cargaHoraria FROM cursos WHERE idCurso={$this->view->id}"; //die($sql);
		$sql = "SELECT cs.titulo, cs.inicio, cs.final, ac.idAluno, ac.idCurso, ac.valorCurso AS valor, ac.parcelas, ac.valorParcelas, cs.cargaHoraria, cs.contrato FROM
		...
		$sql = "SELECT TOP (1)  vcto, idSacado FROM financeiro WHERE
		...
		$this->view->inicio = $this->fc->Ymd2dmY($query->vcto);
...
*/
	...
	function inscricoesAction(){
		...
		if($this->view->id){
			...
			$this->view->combo = $this->fc->dbReader($this->db, "SELECT idCurso, titulo, inicio, contrato FROM cursos ORDER by ativo DESC, inicio DESC", true);		
	...
	function monitoresAction(){
		$this->view->curso = $this->fc->dbReader($this->db, "SELECT titulo FROM cursos WHERE idCurso={$this->view->id}")->titulo;
		$sql = "SELECT ac.*, cd.nome, cd.email, cd.fone, cd.fone2 AS cel 
			FROM (alunosCursos AS ac INNER JOIN cadastro AS cd ON ac.idAluno=cd.idCadastro)
				WHERE (ac.idCurso='$this->id') AND (ac.monitor=1)"; //echo $sql;
		$this->view->monitores = $this->fc->dbReader($this->db, $sql, true);
		//$this->fc->debug($this->view->monitores);die($this->view->curso); 	
		}
