...
class BoletoPDF extends FPDF {
	...
	public function getSetting(){
		$fc = new FuncoesUteis();
		$this->_boleto = $fc->dbReader($this->_dba, "SELECT idCadastro, idSacado, numDoc, historico, valor, vcto FROM financeiro WHERE idFinanceiro={$this->_idBoleto}");
		if($this->_boleto->idSacado > 0)
			$this->_sacado = $fc->dbReader($this->_dba, "SELECT nome, cpf, ender, num, compl, bairro, cep, cidade, uf, email FROM cadastro WHERE idCadastro={$this->_boleto->idSacado}");
		else
