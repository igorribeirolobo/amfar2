 ...
 // debug($rsCurso)
  ?>
	...
	<html>
		...
		<body onload="resizeTo(670,650)" style="background:#ffffff">
	...
	-->
			...
			<?	if (!$rsAluno['idCadastro']) { ?>
				...
				<div id="inscricaoOnLine" <?if($idCurso!=140) echo"style='display:none'";?>><a name='inscricao'><br />
					...
					<form name="cadastro" id="cadastro" style="margin-top:0" action="#cnf" method="POST" onsubmit="return checkForm('cadastro')"/>
						...
						<table cellSpacing="1" cellPadding="1" width="100%" border="0" <?if($idCurso!=140) echo"style='display:none'";?>>
							<tr>
				...
				<form name='confirma' style='width:100%; margin-top:0; border:0; background:#fff' action='inscricao.php' method='POST'  onsubmit="return checkForm('confirma')"/>
					...
					<? if ($rsAluno['idCadastro'] > 0) {
							...
							if ($rsCurso->status ==1) {
								//echo"<div id='dvContrato'><b style=color:red>LEIA COM ATENÇÃO!</b><br/><br/>";
