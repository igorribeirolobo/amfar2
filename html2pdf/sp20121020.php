...
function createpdf($filename, $name){
	...
	<div id="logo">
		<div id="frame">
			<p id="texto">
				Certificamos que <b>' . $name . '</b> participou como CONGRESSISTA do VIII Congresso Brasileiro de Farmácia Hospitalar, 
				promovido pela Sociedade Brasileira de Farmácia Hospitalar e Serviços de Saúde - SBRAFH, realizado no período 
				de 24 a 26 de novembro de 2011, em Salvador – BA.
			</p>
			<p id="carga">
				Carga Horária: 24hs
			</p>
...
try{
	include '../global.php';
	$nome = mssql_result(mssql_query("SELECT nome FROM inscricoesCursos WHERE id={$_GET['id']}"), 0, 0);
	$filename = sprintf('20121020.%06d.pdf', $_GET['id']);
	createpdf($filename, $nome);
	}
catch(Exception $ex){
	echo $ex->getMessage;
	}
